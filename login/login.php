...
<body>
    <main>
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            $blockedDuration = 30; // Block duration in seconds
            ...
            if (!$conn) {
                ...
                if (!$result) {
                    ...
                    if ($result->num_rows > 0) { // Login successful
                        $row = $result->fetch_assoc(); // Fetch the matching record
                        $_SESSION["id"] = $row["Staff_ID"]; // Assuming 'id' corresponds to Staff_ID
                        $staffID = $row["Staff_ID"]; // Get the Staff_ID of the logged-in user
                        ...
                        if ($conn->query($insertQuery) === TRUE) {
                            // Optionally, log that the insertion was successful
                        } else {
